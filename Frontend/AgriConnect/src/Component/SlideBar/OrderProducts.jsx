...
const OrderProducts = () => {
  ...
  useEffect(() => {
    const fetchProducts = async () => {
      try {
        const response = await fetch(`http://localhost:8080/OrderRare/${orderId}`);
        if (!response.ok) {
        ...
        const modifiedData = data.map(product => {
          ...
          const productImage = product.length > 0 && Object.keys(product[0]).length > 0 ? product[0].IMG1 : null;
    ...
    <div>
      ...
      <ul>
        {products.map(product => (
          ...
          (product.productQuantity !== 0 && product.productDescription && product.productTitle) && (
